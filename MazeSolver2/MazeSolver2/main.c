...
const uint16_t THRESHOLD = 0x0460;
...
//void flush(void)
//Pre: should only be called through the analyzePath method
//Post: checks the current buffer for replacements if and only if the size of the stack is exactly 
void flush(void){
	if(buffer[0] == 'S' && buffer[1] == 'U' && buffer[2] == 'L'){
		push(&stk, 'R');
	} else if(buffer[0] == 'L' && buffer[1] == 'U' && buffer[2] == 'S'){
